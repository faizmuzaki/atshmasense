    ...
    <div class="section-body">
      ...
      <div class="row">
        <div class="col-12 col-md-6 col-lg-6">
          <div class="card">
            <form action="<?= base_url('admin/rule/' . $show[0]['id']) ?>" method="post">
              <?= csrf_field() ?>
              ...
              <div class="card-body">
                <div class="form-group">
                  ...
                  <input type="text" class="form-control" name="rule_code" placeholder="Rule Code" value="<?= $show[0]['rule_code'] ?>">
                </div>
                <div class="form-group">
                  <label>Symptoms Name</label>
                  <select class="form-control" aria-label="Default select example" name="symptoms_code">
                    <option value="" selected disabled>Choose Symptoms</option>
                    <?php
                    foreach ($symptoms as $item) {
                    ?>
                      <option value="<?= $item['symptoms_code'] ?>" <?= $show[0]['symptoms_code'] == $item['symptoms_code'] ? 'selected' : '' ?>><?= '[' . $item['symptoms_code'] . '] ' ?><?= $item['symptoms_name'] ?></option>
                    <?php } ?>
